  ...
  describe("shelf/bag", [
    describe("lifecycle", [
      it("opens and closes a bag table", fn() {
        ...
        cleanup(path)
        let assert Ok(table) = bag.open(name: "bag_lifecycle", path: path)
    ...
    describe("read/write", [
      it("stores multiple values per key", fn() {
        ...
        cleanup(path)
        let assert Ok(table) = bag.open(name: "bag_multi", path: path)
      ...
      it("deduplicates identical key-value pairs", fn() {
        ...
        cleanup(path)
        let assert Ok(table) = bag.open(name: "bag_dedup", path: path)
      ...
      it("returns NotFound for missing key", fn() {
        ...
        cleanup(path)
        let assert Ok(table) = bag.open(name: "bag_notfound", path: path)
    ...
    describe("delete", [
      it("delete_object removes one value for a key", fn() {
        ...
        cleanup(path)
        let assert Ok(table) = bag.open(name: "bag_del_obj", path: path)
        let assert Ok(Nil) = bag.insert(table, "color", "red")
        let assert Ok(Nil) = bag.insert(table, "color", "blue")
      ...
      it("delete_key removes all values for a key", fn() {
        ...
        cleanup(path)
        let assert Ok(table) = bag.open(name: "bag_del_key", path: path)
        let assert Ok(Nil) = bag.insert(table, "color", "red")
        let assert Ok(Nil) = bag.insert(table, "color", "blue")
